...
def status():
    ...
    streetcar = {
            ...
            "Portland Streetcar - A Loop":{'target':663, 'count':0},
            "Portland Streetcar - B Loop":{'target':686, 'count':0}
    }
    for record in query:
        debug(record)
...
def data():
    ...
    return render_template('onoff/data.html',
...
def data_query():
    ...
    if csv:
        ...
        for record in data:
            if record:
                #debug(record)
                response += ','.join([(item or '') for item in record])
                response += '\n'
                #debug(response)
                #response += ',' + str(record) + '\n'
